...
当前版本为 0.2.2 / release2。第一版源码继续保留；0.2.2 修复了透明窗口拖动分层、缩放后首条待办无法完成和“清除已完成”失效的问题，不覆盖第一版。
...
- 自由拖动：待办标题和宠物身体都使用 Electron 的 Windows 原生拖动区域，由系统把整扇透明窗口作为一个表面移动；拖动层与备忘框的交互区完全分离。
- 紧凑布局：备忘框会随两个缩放比例保持在宠物头顶附近；拖动不再逐帧重绘分离图层，因此两者不会越拖越远。
- 粉色四角星图标：窗口、托盘和打包程序统一使用清单标题旁粉紫渐变圆角方块 + 白色四角星的图标。
- 稳定交互：release2 不动态关闭窗口鼠标输入，修改窗口与宠物大小后，首条待办和“清除已完成”仍可正常点击。
